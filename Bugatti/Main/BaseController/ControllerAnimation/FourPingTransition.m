...
+ (instancetype)transitionWithTransitionType:(XWPresentOneTransitionType)type point:(CGPoint)point
{
    FourPingTransition *obj = [[self alloc]initWithTransitionType:type];
    obj->point = point;
    return obj;
}
...
//    UIViewController *temp = fromVC.viewControllers.lastObject;
    ...
    UIViewController *temp = (UINavigationController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
...
//    CGFloat y = MAX(button.frame.origin.y, containerView.frame.size.height - button.frame.origin.y);
    CGFloat x = MAX(point.x,containerView.frame.size.width - point.x);
    CGFloat y = MAX(point.y, containerView.frame.size.height - point.y);
    NSLog(@"x:%f  y:%f",x,y);
    //求出半径
...
//    UIViewController *temp = toVC.viewControllers.lastObject;
    UIViewController *temp = (UINavigationController *)[transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    UIView *containerView = [transitionContext containerView];
